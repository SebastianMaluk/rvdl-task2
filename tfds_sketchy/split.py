...
if __name__ == "__main__":
    ...
    photo_folder = os.path.join(".", "rendered_256x256", "256x256", "photo", PHOTO_VER)
    sketch_folder = os.path.join(".", "rendered_256x256", "256x256", "sketch", SKETCH_VER)
    ...
    with open(os.path.join(".", "tensorflow_datasets", "sketchy", "sketchy_classes.json"), "w") as f:
    ...
    valid_unknown_file = open(os.path.join(".", "tensorflow_datasets", "sketchy", "valid_unknown.txt"), "w")
        ...
        for sketch in os.listdir(sketch_dir):
            for photo in selected_photos:
                ...
                if photo_no_ext in sketch:
                    ...
                    negative_class = random.choice(known_classes)
                    while negative_class == current_class:
                        negative_class = random.choice(known_classes)
                    negative_photo_dir = os.path.join(photo_folder, negative_class)
                    negative_photo = random.choice(os.listdir(negative_photo_dir))
                    negative_photo_abs = os.path.abspath(os.path.join(negative_photo_dir, negative_photo))
                    # valid_unknown_file.write(f"{sketch_abs}\t{photo_abs}\t{class_dict[current_class]}\n")
                    valid_unknown_file.write(f"{sketch_abs}\t{photo_abs}\t{negative_photo_abs}\t{class_dict[current_class]}\n")
    valid_unknown_file.close()

    ## DATASET VALID_KNOWN y DATASET TRAIN
    train_file = open(os.path.join(".", "tensorflow_datasets", "sketchy", "train.txt"), "w")
    valid_known_file = open(os.path.join(".", "tensorflow_datasets", "sketchy", "valid_known.txt"), "w")
        ...
        for sketch in os.listdir(sketch_dir):
            ...
            for photo in train_photos:
                ...
                if photo_no_ext in sketch:
                    ...
                    negative_photo = random.choice(os.listdir(negative_photo_dir))
                    negative_photo_abs = os.path.abspath(os.path.join(negative_photo_dir, negative_photo))
                    train_file.write(f"{sketch_abs}\t{photo_abs}\t{negative_photo_abs}\t{class_dict[current_class]}\n")
            ...
            for photo in valid_photos:
                ...
                if photo_no_ext in sketch:
                    ...
                    negative_photo = random.choice(os.listdir(negative_photo_dir))
                    negative_photo_abs = os.path.abspath(os.path.join(negative_photo_dir, negative_photo))
                    valid_known_file.write(f"{sketch_abs}\t{photo_abs}\t{negative_photo_abs}\t{class_dict[current_class]}\n")
    train_file.close()
