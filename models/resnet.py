...
class AttentionBlock(tf.keras.layers.Layer):
    def __init__(
        self, projection_dim, num_heads=4, num_blocks=1, dropout_rate=0.1, **kwargs
    ):
        super(AttentionBlock, self).__init__(**kwargs)
        self.blocks = [
            AttentionLayer(projection_dim, num_heads, dropout_rate)
    ...
    def position_encoding(self, d, n):
        ...
        sins = tf.math.sin(
            pos / tf.math.pow(10000.0, 2.0 * vals_i / tf.cast(d, tf.float32))
        )
        cosins = tf.math.cos(
            pos / tf.math.pow(10000.0, 2.0 * vals_i / tf.cast(d, tf.float32))
        )
...
def create_resnet(config_data, attention=False):
    ...
    if attention:
        model = ResNetAtt([3, 4, 6, 3], [64, 128, 256, 512], n_classes).get_model(
            input_shape
        )
    else:
        model = ResNet([3, 4, 6, 3], [64, 128, 256, 512], n_classes).get_model(
            input_shape
        )
    return model
